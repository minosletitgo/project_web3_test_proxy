...
describe(" ", function () {  
  ...
  before(async function () {
    ...
    //proxyAsLogic = TU_MyContract_V2.attach(proxyAddress);
    // 这里不使用abi，直接生成逻辑合约实例
    proxyAsLogic = await ethers.getContractAt("TU_MyContract_V2", proxyAddress);
...
/*
  npx hardhat test test/test.TU_MyContract_V2.js --network localHardhat
  npx hardhat test test/test.TU_MyContract_V2.js --network sepolia
